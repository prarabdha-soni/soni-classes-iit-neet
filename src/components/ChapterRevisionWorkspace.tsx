...
import { ArrowLeft, ChevronLeft, ChevronRight, Lightbulb, RotateCcw, Check, X, BrainCircuit } from "lucide-react";
import { getChapter, type Subject } from "@/data/content";
import { subjectGradient } from "@/components/SubjectIcon";
import { MathExpr } from "@/components/MathExpr";
import { BookmarkButton } from "@/components/BookmarkButton";
import { useStreak } from "@/hooks/useStreak";

export type ChapterBackLink =
  | { kind: "subject"; subjectId: string; label: string }
  | { kind: "formulasHub"; label: string };
...
export function ChapterRevisionWorkspace({
  subject,
  chapter,
  backLink,
  tabLayoutId = "chapter-revision-tabs",
}: {
  subject: Subject;
  chapter: ChapterData;
  backLink: ChapterBackLink;
  tabLayoutId?: string;
}) {
    ...
    <div className="mx-auto max-w-3xl px-4 pt-6 pb-16">
      {backLink.kind === "formulasHub" ? (
        ...
        <Link
          ...
          params={{ subjectId: backLink.subjectId }}
          className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground"
        >
          <ArrowLeft className="h-4 w-4" /> {backLink.label}
        </Link>
...
function SwipeCard({
  ...
  return (
    <motion.div
      ...
      className="absolute inset-0 cursor-grab touch-none overscroll-x-contain rounded-[2rem] card-soft p-5 sm:p-6 select-none active:cursor-grabbing"
    >
